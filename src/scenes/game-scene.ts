...
export class GameScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.emitter = this.add.particles('flares').createEmitter({
      frame: 'green',
      lifespan: 500,
      angle: { start: 0, end: 360, steps: 20},
      speed: 20,
  ...
  private checkCollision(): void {
    ...
    if (headX === this.apple.x && headY === this.apple.y) {
      ...
      CONST.SCORE++;
      this.emitter.explode(20, this.apple.x + CONST.FIELD_SIZE, this.apple.y + CONST.FIELD_SIZE);

      let scoreText = this.add.text(
        this.apple.x + CONST.FIELD_SIZE,
        this.apple.y + CONST.FIELD_SIZE,
        '+1', { fontSize: '6px' }
      );

      this.tweens.add({
        targets: scoreText,
        props: { y: scoreText.y - 20 },
        duration: 1000,
        ease: 'Power0',
        yoyo: false,
